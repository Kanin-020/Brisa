...
async function bundleCliEntry() {
  ...
  try {
    ...
    // Inyecta la versión de la app para que el CLI empaquetado (modo
    // ELECTRON_RUN_AS_NODE, sin API de Electron) pueda resolverla en el
    // auto-update (src/core/version.ts).
    fs.writeFileSync(
      path.join(tmp, "entry.cjs"),
      `globalThis.__WEB_ASSETS__ = require("./web-assets.cjs");\n` +
        `globalThis.__BRISA_VERSION__ = ${JSON.stringify(pkg.version ?? "0.0.0")};\n` +
