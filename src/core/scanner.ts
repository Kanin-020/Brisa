...
/**
 * Match a single ROM file (any location, e.g. the path Steam ROM Manager
 * passes to `launch`) against all manifests. Returns at most one match per
 * manifest, sorted by reliability (hash > game ID > name), then pattern
 * specificity, then manifest id.
 */
export async function matchRomPath(cfg: AppConfig, file: string): Promise<RomMatch[]> {
  ...
  const sha1 = await hashWithCache(cfg, abs);
  const rom: RomFile = {
    path: abs,
    name: path.basename(abs),
    size: st.size,
    sha1,
    gameId: readDiscGameId(abs),
  };

  const out: RomMatch[] = [];
  for (const m of listManifests(cfg)) {
    for (const req of m.roms) {
      let matchedBy: RomMatch["matchedBy"] | null = null;
      if (req.sha1.length > 0 && req.sha1.includes(rom.sha1)) {
        matchedBy = "hash";
      } else if (
        !matchedBy &&
        (req.gameIds?.length ?? 0) > 0 &&
        rom.gameId !== null &&
        req.gameIds!.includes(rom.gameId)
      ) {
        matchedBy = "gameid";
      } else if (
        !matchedBy &&
        req.sha1.length === 0 &&
        anyMatch(req.patterns, rom.name) &&
        ((req.gameIds?.length ?? 0) === 0 || rom.gameId === null)
      ) {
        matchedBy = "name";
      }
      if (matchedBy) {
        out.push({ manifest: m, requirement: req, rom, matchedBy });
        break; // one match per manifest is enough to route the launch
      }
    }
  }

  const order: Record<RomMatch["matchedBy"], number> = { hash: 0, gameid: 1, name: 2 };
  out.sort((a, b) => {
    const byType = order[a.matchedBy] - order[b.matchedBy];
    if (byType !== 0) return byType;
    const spec =
      patternSpecificity(b.requirement.patterns, b.rom.name) -
      patternSpecificity(a.requirement.patterns, a.rom.name);
