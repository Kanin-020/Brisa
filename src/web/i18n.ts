...
const FALLBACK_EN: Record<string, string | I18nHelpStep[]> = {
  ...
  'port.romOk': 'ROM ready',
  'port.romMissing': 'ROM missing',
  ...
  'mod.close': 'Close',
  'mod.empty': 'This port has no mods yet. Use “Add mods” to add some.',
  ...
  'footer.legal': 'ROMs must be obtained legally',
  'footer.romsDir': 'ROMs',
  'lang.en': 'English',
  'lang.es': 'Español',
};
...
async function fetchLocale(loc: string): Promise<Record<string, string | I18nHelpStep[]> | null> {
  try {
    ...
    // no-store: translation files are NOT content-hashed, so the browser must
    // revalidate them on every load (immutable caching here serves stale strings
    // that render as duplicated glyph+icon or raw emoji fallbacks after updates)
...
async function init(): Promise<void> {
  ...
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 3000);
    const res = await fetch('/api/locales', { signal: controller.signal, cache: 'no-store' });
    clearTimeout(timer);
    if (res.ok) {
