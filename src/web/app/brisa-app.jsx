...
import { BrisaPortCard } from '../components/port-card/port-card.jsx';
import { BrisaRomCard } from '../components/rom-card/rom-card.jsx';
import { BrisaToast } from '../components/toast/toast.jsx';

const { t: tFn, tRaw, setLocale, locale, onLocaleChange, localeLabel, availableLocales } = window.__i18n || {};

/** i18n helper with fallback */
function ti(key, ...args) {
  if (tFn) return tFn(key, ...args);
  return t(key, ...args);
}

/**
 * Main Brisa application component.
 */
export function BrisaApp() {
  const [state, setState] = useState(null);
  const [activeTab, setActiveTab] = useState(() => {
    try { return localStorage.getItem('brisa-active-tab') || 'installed'; } catch { return 'installed'; }
  });
  const [toastMsg, setToastMsg] = useState('');
  const [toastKind, setToastKind] = useState('ok');
  const [busyPorts, setBusyPorts] = useState(new Set());
  ...
  const [currentLocale, setCurrentLocale] = useState(() => {
    if (locale) try { return locale(); } catch { /* ignore */ }
    return 'en';
  });
  ...
  // ── Subscribe to locale changes so the UI re-renders ──
  useEffect(() => {
    if (onLocaleChange) {
  ...
  // ── Toast ──
  const showToast = useCallback((msg, kind = 'ok', duration = 3200) => {
    setToastMsg('');
    ...
    try {
      ...
      for (const [taskId, entry] of activeTasks.current) {
        ...
        if (!info) {
          ...
          setBusyPorts(prev => { const next = new Set(prev); next.delete(entry.portId); return next; });
          continue;
        }
  ...
  // ── Track a new task ──
  const trackTask = useCallback((task, portId, onDone = null) => {
    activeTasks.current.set(task.id, { task, portId, onDone });
    setBusyPorts(prev => new Set([...prev, portId]));
    startPolling();
  }, [startPolling]);

  // ── API actions ──
  const installPort = useCallback(async (port) => {
    ...
    try {
      ...
      showToast(err.message, 'error');
      setBusyPorts(prev => { const next = new Set(prev); next.delete(portId); return next; });
    }
  }, [trackTask]);

  const updatePort = useCallback(async (port, launchAfter = false) => {
    const portId = port.manifest.id;
    setBusyPorts(prev => new Set([...prev, portId]));
    showToast(ti('toast.installing', port.manifest.name), 'ok');
    try {
      ...
      const data = await res.json();
      if (data.task) trackTask(data.task, portId, launchAfter ? () => launchPort(port) : null);
    } catch (err) {
      showToast(err.message, 'error');
      setBusyPorts(prev => { const next = new Set(prev); next.delete(portId); return next; });
    }
  ...
  const launchPort = useCallback(async (port) => {
    try {
  ...
  const uninstallPort = useCallback(async (port) => {
    ...
    try {
      ...
      setBusyPorts(prev => { const next = new Set(prev); next.delete(port.manifest.id); return next; });
  ...
  const toggleMod = useCallback(async (portId, mod, isLinked) => {
  ...
  // ── ROM upload ──
  const uploadRom = useCallback((file, onProgress) => {
  ...
  const uploadRoms = useCallback(async (files) => {
    let added = 0;
    let skipped = 0;
    for (const file of files) {
      if (!file.name || file.size === 0) continue;
      try {
        const data = await uploadRom(file, (pct) => showToast(`${ti('toast.uploading', file.name)} ${pct}%`));
    ...
    if (added > 0 && skipped > 0) {
      showToast(`${ti('toast.romsAdded', added)} · ${ti('toast.romsSkipped', skipped)}`, 'ok', 5000);
    } else if (added > 0) {
  ...
  const openPortFolder = useCallback(async (port) => {
    try {
  ...
  const openPortModsFolder = useCallback(async (port) => {
    try {
  ...
  const switchTab = useCallback((tab) => {
    setActiveTab(tab);
    try { localStorage.setItem('brisa-active-tab', tab); } catch { /* ignore */ }
  }, []);
  ...
  const filterPorts = (portsList, query) => {
  ...
  // ── Help steps ──
  const helpSteps = tRaw ? (tRaw('help.steps') || []) : [];
  ...
  return (
    <div class="app">
      ...
      <header class="topbar">
        <div class="brand">
          <div class="logo"><img src="/icon.png" alt="Brisa" onerror={(e) => e.target.remove()} /></div>
          <div>
            <h1 id="brand-title">{ti('brand.title')}</h1>
            <span id="brand-tagline" class="tagline">{ti('brand.tagline')}</span>
          </div>
        </div>
        <div class="topbar-right">
          <button class="btn ghost" onClick={loadState}>{ti('btn.refresh')}</button>
          <button class="btn ghost sm settings-btn" title={ti('settings.title')} onClick={() => setSettingsOpen(true)}>⚙️</button>
        </div>
      </header>
      ...
      <div class="tabs-row">
        <nav class="tabs" role="tablist">
          {[
            { id: 'installed', label: ti('tabs.installed'), count: installed.length },
            { id: 'available', label: ti('tabs.available'), count: available.length },
            { id: 'roms', label: ti('tabs.roms'), count: scan.roms.length },
            { id: 'help', label: ti('tabs.help') },
          ].map(tab => (
            <button
              key={tab.id}
      ...
      <main>
        ...
        {['installed', 'available'].map(tabId => {
          ...
          return (
            ...
            >
              ...
              <div id={`ports-grid-${tabId}`} class="grid">
                {list.length === 0 ? (
        ...
        >
          ...
          <div id="roms-list" class="roms-grid">
            {scan.roms.length === 0 ? (
              <div class="loading">{ti('roms.empty')}</div>
            ) : (
              scan.roms.map((rom, i) => {
                const matchedPorts = scan.matches
        ...
        >
          ...
          <p id="help-intro" class="help-intro">{ti('help.intro')}</p>
          <div id="help-steps" class="help-grid">
            ...
            <div class="help-foot">
              ...
              <button class="btn green" onClick={() => switchTab('installed')}>{ti('help.finish')}</button>
            </div>
          </div>
        </section>
      </main>
      ...
      {/* Settings Modal — always in the DOM, toggled via .show (like original) */}
      <div id="settings-modal" class={`modal-overlay ${settingsOpen ? 'show' : ''}`} role="dialog" aria-modal="true"
        onClick={(e) => { if (e.target === e.currentTarget) setSettingsOpen(false); }}>
        <div class="modal modal-settings">
          <div class="modal-head">
            <h3 id="settings-modal-title">{ti('settings.title')}</h3>
            <button id="settings-modal-close" class="modal-close" title={ti('settings.close')} onClick={() => setSettingsOpen(false)}>✕</button>
          </div>
          <div id="settings-modal-body" class="modal-body">
            ...
            <div class="settings-group">
              ...
              <div class="seg" id="settings-lang">
                {(availableLocales ? availableLocales() : []).map(loc => (
                  <button
                    key={loc}class={`seg-btn ${currentLocale === loc ? 'active' : ''}`}
                      data-key="locale"
                      data-value={loc}
                      aria-pressed={currentLocale === loc}
                      onClick={() => { if (setLocale) setLocale(loc); }}
                  >
                    {localeLabel ? localeLabel(loc) : loc}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div class="settings-group">
              ...
              <div class="seg" id="settings-theme">
                ...
                >☀️ {ti('settings.themeLight')}</button>
                <button
                  class={`seg-btn ${currentTheme === 'dark' ? 'active' : ''}`}
                  data-value="dark"
                  aria-pressed={currentTheme === 'dark'}
                  onClick={() => applyTheme('dark')}
                >🌙 {ti('settings.themeDark')}</button>
              </div>
            </div>
          </div>
        </div>
