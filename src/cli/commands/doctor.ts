...
export function registerDoctorCommand(program: Command, app: App): void {
  program
    ...
    .action(async () => {
      ...
      // ── Node / entorno ──
      c.check(`Node.js ${process.version} (mínimo ${MIN_NODE_VERSION})`, nodeVersionOk(process.version));
      ...
      if (installed.length > 0) {
        ...
        if (missingDir.length > 0) {
          c.check(`Carpetas de ports instalados`, false, `${missingDir.length} sin carpeta (reinstala: brisa install)`);
        } else {
      ...
      if (c.fails.length > 0) {
        ...
        console.log(`✓ Entorno OK${c.warnings.length > 0 ? ` (${c.warnings.length} aviso(s))` : ""}`);
      }
    });
}
