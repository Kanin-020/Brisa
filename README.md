...
Soporta **SoH** (Ocarina of Time), **2Ship2Harkinian** (Majora's Mask), **DUSKLIGHT** (Twilight Princess) y **Project Picori** (Minish Cap), y es trivialmente extensible a cualquier port con releases en GitHub.
...
|---|-----------|--------------|
| 1 | **Un solo directorio de ROMs** | Todos los ROMs viven en `roms/`. Al instalar un port, Port Hub crea el **symlink** con el nombre exacto que el juego espera (`oot.z64`, `mm.z64`, `tp.iso`, `tmc.gba`) dentro de su carpeta. Los ports **multirom** (SoH: base + Master Quest) enlazan una ROM por requisito automáticamente. |
| 2 | **Escaneo automático al abrir** | `port-hub status` (y la GUI web) escanean `roms/`, calculan **SHA1** (con caché), asocian cada ROM a su port por nombre *y* por hash, y te dice qué ports puedes instalar. |
...
1. **Escaneo**: recorre `roms/`, calcula SHA1 con caché por tamaño+mtime, y para cada manifiesto busca su ROM por nombre (patrones) y/o hash exacto.
...
| `2ship2harkinian` | Majora's Mask | `HarbourMasters/2ship2harkinian` | MM NTSC 1.0 (`.z64`/`.n64`) |
| `dusklight` | Twilight Princess | `TwilitRealm/dusklight` | TP NTSC-U GameCube ISO (`.iso`/`.gcz`) |
